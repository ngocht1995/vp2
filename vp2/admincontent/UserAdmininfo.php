...
// PHPMaker 6 configuration for Table UserAdmin
$UserAdmin = NULL; // Initialize table object
...
class cUserAdmin {
	...
	function cUserAdmin() {
		$this->TableVar = "UserAdmin";
		$this->TableName = "UserAdmin";
		$this->nguoidung_id = new cField('UserAdmin', 'x_nguoidung_id', 'nguoidung_id', "user.nguoidung_id", 19, -1, FALSE);
		$this->fields['nguoidung_id'] =& $this->nguoidung_id;
		$this->nguoidung_option = new cField('UserAdmin', 'x_nguoidung_option', 'nguoidung_option', "user.nguoidung_option", 19, -1, FALSE);
		$this->fields['nguoidung_option'] =& $this->nguoidung_option;
		$this->tendangnhap = new cField('UserAdmin', 'x_tendangnhap', 'tendangnhap', "user.tendangnhap", 200, -1, FALSE);
		$this->fields['tendangnhap'] =& $this->tendangnhap;
		$this->mat_khau = new cField('UserAdmin', 'x_mat_khau', 'mat_khau', "user.mat_khau", 200, -1, FALSE);
		$this->fields['mat_khau'] =& $this->mat_khau;
		$this->truycap_gannhat = new cField('UserAdmin', 'x_truycap_gannhat', 'truycap_gannhat', "user.truycap_gannhat", 135, 7, FALSE);
		$this->fields['truycap_gannhat'] =& $this->truycap_gannhat;
		$this->UserLevelID = new cField('UserAdmin', 'x_UserLevelID', 'UserLevelID', "user.UserLevelID", 3, -1, FALSE);
		$this->fields['UserLevelID'] =& $this->UserLevelID;
		$this->hoten_nguoilienhe = new cField('UserAdmin', 'x_hoten_nguoilienhe', 'hoten_nguoilienhe', "user.hoten_nguoilienhe", 200, -1, FALSE);
		$this->fields['hoten_nguoilienhe'] =& $this->hoten_nguoilienhe;
		$this->gioi_tinh = new cField('UserAdmin', 'x_gioi_tinh', 'gioi_tinh', "user.gioi_tinh", 19, -1, FALSE);
		$this->fields['gioi_tinh'] =& $this->gioi_tinh;
		$this->quocgia_id = new cField('UserAdmin', 'x_quocgia_id', 'quocgia_id', "user.quocgia_id", 200, -1, FALSE);
		$this->fields['quocgia_id'] =& $this->quocgia_id;
		$this->nick_yahoo = new cField('UserAdmin', 'x_nick_yahoo', 'nick_yahoo', "user.nick_yahoo", 200, -1, FALSE);
		$this->fields['nick_yahoo'] =& $this->nick_yahoo;
		$this->nick_skype = new cField('UserAdmin', 'x_nick_skype', 'nick_skype', "user.nick_skype", 200, -1, FALSE);
	...
	function SqlSelect() { // Select
		return "SELECT user.nguoidung_id, user.nguoidung_option, user.tendangnhap, user.mat_khau, user.truycap_gannhat, user.UserLevelID, user.hoten_nguoilienhe, user.gioi_tinh, user.quocgia_id, user.nick_yahoo, user.nick_skype FROM user";
	}

	function SqlWhere() { // Where
		return "user.nguoidung_option = 0";
	}
...
// INSERT statement
	...
	// View url
	function ViewUrl() {
		return $this->KeyUrl("UserAdminview.php", $this->UrlParm());
	}

	// Add url
	function AddUrl() {
		$AddUrl = "useradminadd.php";
	...
	// Edit url
	function EditUrl() {
		return $this->KeyUrl("UserAdminedit.php", $this->UrlParm());
	}
	...
	// Copy url
	function CopyUrl() {
		return $this->KeyUrl("userudminadd.php", $this->UrlParm());
	}
	...
	// Delete url
	function DeleteUrl() {
		return $this->KeyUrl("UserAdmindelete.php", $this->UrlParm());
	}
	...
	function KeyUrl($url, $parm = "") {
		...
		if (!is_null($this->nguoidung_id->CurrentValue)) {
			...
			return "javascript:alert('Bản ghi không hợp lệ.Khóa có giá trị null');";
		}
		return $sUrl;
	}
	...
	function UrlParm($parm = "") {
		$UrlParm = ($this->UseTokenInUrl) ? "t=UserAdmin" : "";
		if ($parm <> "") {
			if ($UrlParm <> "")
				$UrlParm .= "&";
			$UrlParm .= $parm;
	...
	function RenderListRow() {
		...
		if (strval($this->nguoidung_option->CurrentValue) <> "") {
			switch ($this->nguoidung_option->CurrentValue) {
				case "0":
					$this->nguoidung_option->ViewValue = "Quản lý hệ thống";
					break;
				case "1":
					$this->nguoidung_option->ViewValue = "Thành viên đăng ký";
					break;
				default:
