...
// Define page object
$offer_edit = new coffer_edit();
$Page =& $offer_edit;

// Page init processing
$offer_edit->Page_Init();

// Page main processing
$offer_edit->Page_Main();
?>
...
// Create page object
var offer_edit = new ew_Page("offer_edit");

// page properties
offer_edit.PageID = "edit"; // page ID
var EW_PAGE_ID = offer_edit.PageID; // for backward compatibility

// extend page with ValidateForm function
offer_edit.ValidateForm = function(fobj) {
...
offer_edit.Form_CustomValidate = 
 function(fobj) { // DO NOT CHANGE THIS LINE!

 	// Your custom validation code here, return false if invalid. 
 	return true;
 }
offer_edit.SelectAllKey = function(elem) {
	ew_SelectAll(elem);
}
<?php if (EW_CLIENT_VALIDATE) { ?>
offer_edit.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
offer_edit.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
<table border="0" width="100%" id="table806" cellspacing="0" cellpadding="0">
							<tr>
								...
								&nbsp;&nbsp;&nbsp;Sửa thông tin chào hàng</font></b></td>
...
<?php $offer_edit->ShowMessage() ?>
<form name="fofferedit" id="fofferedit" action="<?php echo ew_CurrentPage() ?>" method="post" enctype="multipart/form-data" onsubmit="return offer_edit.ValidateForm(this);">
<p>
<input type="hidden" name="a_table" id="a_table" value="offer">
...
<div id="new_x_anh_chaohang">
<?php if (!is_null($offer->anh_chaohang->Upload->DbValue)) { ?>
<input type="radio" name="a_anh_chaohang" id="a_anh_chaohang" value="1" checked="checked">Bỏ qua&nbsp;
<input type="radio" name="a_anh_chaohang" id="a_anh_chaohang" value="2">Xóa&nbsp;
<input type="radio" name="a_anh_chaohang" id="a_anh_chaohang" value="3">Thay thế<br>
<?php } else { ?>
<input type="hidden" name="a_anh_chaohang" id="a_anh_chaohang" value="3">
<?php } ?>
<input type="file" name="x_anh_chaohang" id="x_anh_chaohang" size="30" onchange="if (this.form.a_anh_chaohang[2]) this.form.a_anh_chaohang[2].checked=true;"<?php echo $offer->anh_chaohang->EditAttributes() ?>>
</div>
...
<input type="hidden" name="x_chaohang_id" id="x_chaohang_id" value="<?php echo ew_HtmlEncode($offer->chaohang_id->CurrentValue) ?>">
<p>
<input type="button" name="btnAction" id="btnAction" value="   Cập nhật   " onclick="ew_SubmitForm(offer_edit, this.form);">
</form>
<script type="text/javascript">
<!--
...
class coffer_edit {
	...
	function Page_Main() {
		...
		if (@$_POST["a_edit"] <> "") {
			$offer->CurrentAction = $_POST["a_edit"]; // Get action code
			$this->GetUploadFiles(); // Get upload files
			$this->LoadFormValues(); // Get form values

			// Validate Form
			if (!$this->ValidateForm()) {
				$offer->CurrentAction = ""; // Form error, reset action
				$this->setMessage($gsFormError);
				$this->RestoreFormValues();
			}
		} else {
			$offer->CurrentAction = "I"; // Default action is display
		}

		// Check if valid key
		if ($offer->chaohang_id->CurrentValue == "")
			$this->Page_Terminate("offerlist.php"); // Invalid key, return to list
		switch ($offer->CurrentAction) {
			case "I": // Get a record to display
				...
				break;
			Case "U": // Update
				$offer->SendEmail = TRUE; // Send email on update success
				if ($this->EditRow()) { // Update record based on key
					$this->setMessage("Đã Sửa chào hàng"); // Update success
		...
		// Render the record
		$offer->RowType = EW_ROWTYPE_EDIT; // Render as edit
		$this->RenderRow();
	}
	...
	function RenderRow() {
		...
		if ($offer->RowType == EW_ROWTYPE_VIEW) { // View row
			...
			// anh_chaohang
			if (!is_null($offer->anh_chaohang->Upload->DbValue)) {
				$offer->anh_chaohang->HrefValue = ew_UploadPathEx(FALSE, EW_UPLOAD_DEST_PATH) . ((!empty($offer->anh_chaohang->EditValue)) ? $offer->anh_chaohang->EditValue : $offer->anh_chaohang->CurrentValue);
				if ($offer->Export <> "") $offer->anh_chaohang->HrefValue = ew_ConvertFullUrl($offer->anh_chaohang->HrefValue);
			} else {
				$offer->anh_chaohang->HrefValue = "";
			}
	...
	function ValidateForm() {
		...
		if ($offer->noidung_chaohang->FormValue == "") {
			$gsFormError .= ($gsFormError <> "") ? "<br>" : "";
			$gsFormError .= "Chưa nhập - Noidung Chaohang";
		}*/
			$img = new Securimage();
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			$offer->anh_chaohang->Upload->DbValue = $rs->fields('anh_chaohang'); // Get original value
			if (is_null($offer->anh_chaohang->Upload->Value)) {
				$rsnew['anh_chaohang'] = NULL;
			} else {
				if ($offer->anh_chaohang->Upload->FileName == $offer->anh_chaohang->Upload->DbValue) { // Upload file name same as old file name
					$rsnew['anh_chaohang'] = $offer->anh_chaohang->Upload->FileName;
				} else {
			...
			// Field nganhnghe_id
			$offer->nganhnghe_id->SetDbValueDef($offer->nganhnghe_id->CurrentValue, NULL);
			$rsnew['nganhnghe_id'] =& $offer->nganhnghe_id->DbValue;

			// Field thoihan_tungay
			$offer->thoihan_tungay->SetDbValueDef(ew_UnFormatDateTime($offer->thoihan_tungay->CurrentValue, 7), NULL);
			$rsnew['thoihan_tungay'] =& $offer->thoihan_tungay->DbValue;

			// Field thoihan_denngay
			$offer->thoihan_denngay->SetDbValueDef(ew_UnFormatDateTime($offer->thoihan_denngay->CurrentValue, 7), NULL);
			...
			$offer->tg_suachaohang->SetDbValueDef(ew_CurrentDateTime(), NULL);
			$rsnew['tg_suachaohang'] =& $offer->tg_suachaohang->DbValue;

			// Field xuat_su
			$offer->xuat_su->SetDbValueDef($offer->xuat_su->CurrentValue, NULL);
