...
// Define page object
$subject_update = new csubject_update();
$Page =& $subject_update;

// Page init processing
$subject_update->Page_Init();

// Page main processing
$subject_update->Page_Main();
?>
...
// Create page object
var subject_update = new ew_Page("subject_update");

// page properties
subject_update.PageID = "update"; // page ID
var EW_PAGE_ID = subject_update.PageID; // for backward compatibility

// extend page with ValidateForm function
subject_update.ValidateForm = function(fobj) {
	...
	for (i=0; i<rowcnt; i++) {
		...
		if (uelm && uelm.checked) {
			if (elm && !ew_HasValue(elm))
				return ew_OnError(this, elm, "Chưa nhập - Trang Thai");
		}
...
subject_update.Form_CustomValidate =
 function(fobj) { // DO NOT CHANGE THIS LINE!

 	// Your custom validation code here, return false if invalid. 
 	return true;
 }
subject_update.SelectAllKey = function(elem) {
	ew_SelectAll(elem);
}
<?php if (EW_CLIENT_VALIDATE) { ?>
subject_update.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
subject_update.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
<?php $subject_update->ShowMessage() ?>
<form name="fsubjectupdate" id="fsubjectupdate" action="<?php echo ew_CurrentPage() ?>" method="post" onsubmit="return subject_update.ValidateForm(this);">
<p>
<input type="hidden" name="t" id="t" value="subject">
<input type="hidden" name="a_update" id="a_update" value="U">
<?php for ($i = 0; $i < $subject_update->nKeySelected; $i++) { ?>
<input type="hidden" name="k<?php echo $i+1 ?>_key" id="key<?php echo $i+1 ?>" value="<?php echo ew_HtmlEncode($subject_update->arRecKeys[$i]) ?>">
<?php } ?>
...
<?php if ($subject->trang_thai->Visible) { // trang_thai ?>
	<tr<?php echo $subject->trang_thai->RowAttributes ?>>
		<td<?php echo $subject->trang_thai->CellAttributes() ?>>
<input type="hidden" name="u_trang_thai" id="u_trang_thai" value="1">
Trạng thái</td>
		<td<?php echo $subject->trang_thai->CellAttributes() ?>><span id="el_trang_thai">
<select id="x_trang_thai" name="x_trang_thai"<?php echo $subject->trang_thai->EditAttributes() ?>>
<?php
if (is_array($subject->trang_thai->EditValue)) {
	$arwrk = $subject->trang_thai->EditValue;
	$rowswrk = count($arwrk);
	$emptywrk = TRUE;
	for ($rowcntwrk = 0; $rowcntwrk < $rowswrk; $rowcntwrk++) {
		$selwrk = (strval($subject->trang_thai->CurrentValue) == strval($arwrk[$rowcntwrk][0])) ? " selected=\"selected\"" : "";
...
</span><?php echo $subject->trang_thai->CustomMsg ?></td>
	</tr>
<?php } ?>
</table>
</div>
...
class csubject_update {
	...
	// Page Object Name
	var $PageObjName = 'subject_update';
	...
	function Page_Main() {
		...
		if ($this->nKeySelected == 0) {
			...
			if (@$_POST["a_update"] <> "") {
				...
				// Get action
				$subject->CurrentAction = $_POST["a_update"];
		...
		if ($this->nKeySelected <= 0)
			$this->Page_Terminate("subjectlist.php"); // No records selected, return to list
	...
	function LoadMultiUpdateValues() {
		global $subject;
		$subject->CurrentFilter = $this->BuildKeyFilter();
		...
		while (!$rs->EOF) {
			if ($i == 1) {
				...
				if (!ew_CompareValue($subject->trang_thai->DbValue, $rs->fields('trang_thai')))
					$subject->trang_thai->CurrentValue = NULL;
			}
			$i++;
			$rs->MoveNext();
		}
	...
	function SetupKeyValues($key) {
		...
		if (!is_numeric($sKeyFld))
			return FALSE;
		$subject->chuyenmuc_id->CurrentValue = $sKeyFld;
		return TRUE;
	}
	...
	function UpdateRows() {
		...
		foreach ($this->arRecKeys as $sThisKey) {
			...
			if ($this->SetupKeyValues($sThisKey)) {
				$subject->SendEmail = FALSE; // Do not send email on update success
	...
	function LoadFormValues() {
		...
		// Load from form
		global $objForm, $subject;
		$subject->trang_thai->setFormValue($objForm->GetValue("x_trang_thai"));
		$subject->trang_thai->MultiUpdate = $objForm->GetValue("u_trang_thai");
	...
	function RestoreFormValues() {
		...
		$subject->trang_thai->CurrentValue = $subject->trang_thai->FormValue;
	}
	...
	function ValidateForm() {
		...
		$gsFormError = "";
		$lUpdateCnt = 0;
		if ($subject->trang_thai->MultiUpdate == "1") $lUpdateCnt++;
		if ($lUpdateCnt == 0) {
		...
		if (!EW_SERVER_VALIDATE)
			return ($gsFormError == "");
		if ($subject->trang_thai->MultiUpdate <> "" && $subject->trang_thai->FormValue == "") {
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// Field trang_thai
			if ($subject->trang_thai->MultiUpdate == "1") {
			$subject->trang_thai->SetDbValueDef($subject->trang_thai->CurrentValue, 0);
			$rsnew['trang_thai'] =& $subject->trang_thai->DbValue;
			}
