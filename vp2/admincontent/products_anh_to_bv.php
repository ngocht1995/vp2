...
// Define page object
$products_anh_to_blobview = new cproducts_anh_to_blobview();
$Page =& $products_anh_to_blobview;

// Page init processing
$products_anh_to_blobview->Page_Init();

// Page main processing
$products_anh_to_blobview->Page_Main();
?>
<?php
...
// Page Class
//
class cproducts_anh_to_blobview {

	// Page ID
	var $PageID = 'blobview';

	// Page Object Name
	var $PageObjName = 'products_anh_to_blobview';
	...
	//  - open connection
	//
	function cproducts_anh_to_blobview() {
		global $conn;
	...
	function Page_Main() {
		...
		$objBinary = new cUpload('products', 'x_anh_to');
		...
		if ($rs = $conn->Execute($sSql)) {
			if (!$rs->EOF) {
				...
				if (trim(strval($rs->fields('anh_to'))) <> "") {
					header("Content-Disposition: attachment; filename=" . $rs->fields('anh_to'));
				}
				$objBinary->Value = $rs->fields('anh_to');
				if ($bShowThumbnail) {
