...
// Define page object
$pic_pic_blobview = new cpic_pic_blobview();
$Page =& $pic_pic_blobview;

// Page init processing
$pic_pic_blobview->Page_Init();

// Page main processing
$pic_pic_blobview->Page_Main();
?>
<?php
...
// Page Class
//
class cpic_pic_blobview {

	// Page ID
	var $PageID = 'blobview';

	// Page Object Name
	var $PageObjName = 'pic_pic_blobview';
	...
	//  - open connection
	//
	function cpic_pic_blobview() {
		global $conn;

		// Initialize table object
		$GLOBALS["pic"] = new cpic();
	...
	//
	function Page_Init() {
		global $gsExport, $gsExportFile, $pic;
		global $Security;
		...
		$Security->LoadCurrentUserLevel('pic');
	...
	function Page_Main() {
		...
		// Get key
		if (@$_GET["anh_id"] <> "") {
			$pic->anh_id->setQueryStringValue($_GET["anh_id"]);
		} else {
			$this->Page_Terminate(); // Exit
			exit();
		}
		$objBinary = new cUpload('pic', 'x_pic');
		...
		$sFilter = $pic->KeyFilter();

		// Set up filter (SQL WHERE clause) and get return SQL
		// SQL constructor in pic class, picinfo.php

		$pic->CurrentFilter = $sFilter;
		$sSql = $pic->SQL();
		if ($rs = $conn->Execute($sSql)) {
			if (!$rs->EOF) {
				if (strpos(ew_ServerVar("HTTP_USER_AGENT"), "MSIE") === FALSE)
					header("Content-type: images");
				$objBinary->Value = $rs->fields('pic');
				if ($bShowThumbnail) {
