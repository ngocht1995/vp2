...
// Define page object
$advertising_info_edit = new cadvertising_info_edit();
$Page =& $advertising_info_edit;

// Page init processing
$advertising_info_edit->Page_Init();

// Page main processing
$advertising_info_edit->Page_Main();
?>
...
// Create page object
var advertising_info_edit = new ew_Page("advertising_info_edit");

// page properties
advertising_info_edit.PageID = "edit"; // page ID
var EW_PAGE_ID = advertising_info_edit.PageID; // for backward compatibility

// extend page with ValidateForm function
advertising_info_edit.ValidateForm = function(fobj) {
...
advertising_info_edit.Form_CustomValidate = 
 function(fobj) { // DO NOT CHANGE THIS LINE!

 	// Your custom validation code here, return false if invalid. 
 	return true;
 }
advertising_info_edit.SelectAllKey = function(elem) {
	ew_SelectAll(elem);
}
<?php if (EW_CLIENT_VALIDATE) { ?>
advertising_info_edit.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
advertising_info_edit.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
<?php $advertising_info_edit->ShowMessage() ?>
<form name="fadvertising_infoedit" id="fadvertising_infoedit" action="<?php echo ew_CurrentPage() ?>" method="post"  enctype="multipart/form-data" onsubmit="return advertising_info_edit.ValidateForm(this);">
<p>
<input type="hidden" name="a_table" id="a_table" value="advertising_info">
...
<div id="old_x_anh_daidien">
<?php if ($advertising_info->anh_daidien->HrefValue <> "") { ?>
<?php if (!is_null($advertising_info->anh_daidien->Upload->DbValue)) { ?>
<a href="<?php echo $advertising_info->anh_daidien->HrefValue ?>" target="_blank"><img src="advertising_info_anh_daidien_bv.php?baiviet_id=<?php echo $advertising_info->baiviet_id->CurrentValue ?>" border=0<?php echo $advertising_info->anh_daidien->ViewAttributes() ?>></a>
<?php } elseif (!in_array($advertising_info->CurrentAction, array("I", "edit", "gridedit"))) { ?>
&nbsp;
<?php } ?>
<?php } else { ?>
<?php if (!is_null($advertising_info->anh_daidien->Upload->DbValue)) { ?>
<img src="advertising_info_anh_daidien_bv.php?baiviet_id=<?php echo $advertising_info->baiviet_id->CurrentValue ?>" border=0<?php echo $advertising_info->anh_daidien->ViewAttributes() ?>>
<?php } elseif (!in_array($advertising_info->CurrentAction, array("I", "edit", "gridedit"))) { ?>
&nbsp;
<?php } ?>
<?php } ?>
</div>
<div id="new_x_anh_daidien">
<?php if (!is_null($advertising_info->anh_daidien->Upload->DbValue)) { ?>
<input type="radio" name="a_anh_daidien" id="a_anh_daidien" value="1" checked="checked"><?php echo Lang_Text('Bỏ qua');?>&nbsp;
<input type="radio" name="a_anh_daidien" id="a_anh_daidien" value="2"><?php echo Lang_Text('Xóa');?>&nbsp;
<input type="radio" name="a_anh_daidien" id="a_anh_daidien" value="3"><?php echo Lang_Text('Thay thế');?><br>
<?php } else { ?>
<input type="hidden" name="a_anh_daidien" id="a_anh_daidien" value="3">
<?php } ?>
<input type="file" name="x_anh_daidien" id="x_anh_daidien" size="30" onchange="if (this.form.a_anh_daidien[2]) this.form.a_anh_daidien[2].checked=true;"<?php echo $advertising_info->anh_daidien->EditAttributes() ?>>
</div>
...
<?php if ($advertising_info->thutu_sapxep->Visible) { // thutu_sapxep ?>
	<tr<?php echo $advertising_info->thutu_sapxep->RowAttributes ?>>
		...
		<td<?php echo $advertising_info->thutu_sapxep->CellAttributes() ?>><span id="el_thutu_sapxep">
                        <select id="x_thutu_sapxep" name="x_thutu_sapxep"<?php echo ew_TruncateMemo($advertising_info->thutu_sapxep->EditAttributes(), 100)  ?>>
<?php
...
<input type="hidden" name="x_baiviet_id" id="x_baiviet_id" value="<?php echo ew_HtmlEncode($advertising_info->baiviet_id->CurrentValue) ?>">
<p>
<input type="button" name="btnAction" id="btnAction" value="   <?php echo Lang_Text('Sửa');?>   " onclick="ew_SubmitForm(advertising_info_edit, this.form);">
</form>
<script type="text/javascript">
<!--
...
class cadvertising_info_edit {
	...
	// Page Object Name
	var $PageObjName = 'advertising_info_edit';
	...
	//  - open connection
	//
	function cadvertising_info_edit() {
		global $conn;
	...
	function Page_Main() {
		...
		if (@$_POST["a_edit"] <> "") {
			$advertising_info->CurrentAction = $_POST["a_edit"]; // Get action code
                        $this->GetUploadFiles(); // Get upload files
			$this->LoadFormValues(); // Get form values

			// Validate Form
			if (!$this->ValidateForm()) {
				$advertising_info->CurrentAction = ""; // Form error, reset action
				$this->setMessage($gsFormError);
				$this->RestoreFormValues();
			}
		} else {
			$advertising_info->CurrentAction = "I"; // Default action is display
		}

		// Check if valid key
		if ($advertising_info->baiviet_id->CurrentValue == "")
			$this->Page_Terminate("advertising_infolist.php"); // Invalid key, return to list
		switch ($advertising_info->CurrentAction) {
			case "I": // Get a record to display
				...
				break;
			Case "U": // Update
				$advertising_info->SendEmail = TRUE; // Send email on update success
		...
		// Render the record
		$advertising_info->RowType = EW_ROWTYPE_EDIT; // Render as edit
		$this->RenderRow();
	}
	...
	function ValidateForm() {
		...
		/*if ($advertising_info->chuyenmuc_id->FormValue == "") {
			$gsFormError .= ($gsFormError <> "") ? "<br>" : "";
			$gsFormError .= "<?php echo Lang_Text('Hãy nhập trường bắt buộc');?> - Chuyên mục";
		}
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			if ($advertising_info->anh_daidien->Upload->Action == "2" || $advertising_info->anh_daidien->Upload->Action == "3") { // Update/Remove
