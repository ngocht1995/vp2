...
// Define page object
$Search_offer_anh_chaohang_blobview = new cSearch_offer_anh_chaohang_blobview();
$Page =& $Search_offer_anh_chaohang_blobview;

// Page init processing
$Search_offer_anh_chaohang_blobview->Page_Init();

// Page main processing
$Search_offer_anh_chaohang_blobview->Page_Main();
?>
<?php
...
// Page Class
//
class cSearch_offer_anh_chaohang_blobview {

	// Page ID
	var $PageID = 'blobview';

	// Page Object Name
	var $PageObjName = 'Search_offer_anh_chaohang_blobview';
	...
	//  - open connection
	//
	function cSearch_offer_anh_chaohang_blobview() {
		global $conn;
	...
	function Page_Init() {
		...
		$Security->LoadCurrentUserLevel('Search_offer');
	...
	function Page_Main() {
		...
		$objBinary = new cUpload('Search_offer', 'x_anh_chaohang');

		// Show thumbnail
		$bShowThumbnail = (@$_GET["showthumbnail"] == "1");
		if (@$_GET["thumbnailwidth"] == "" && @$_GET["thumbnailheight"] == "") {
			$iThumbnailWidth = 0; // Set default width
		...
		// SQL constructor in Search_offer class, Search_offerinfo.php
		...
		if ($rs = $conn->Execute($sSql)) {
			if (!$rs->EOF) {
				...
				if (trim(strval($rs->fields('anh_chaohang'))) <> "") {
					header("Content-Disposition: attachment; filename=" . $rs->fields('anh_chaohang'));
				}
				$objBinary->Value = $rs->fields('anh_chaohang');
				if ($bShowThumbnail) {
