...
// Define page object
$help_manager_edit = new chelp_manager_edit();
$Page =& $help_manager_edit;

// Page init processing
$help_manager_edit->Page_Init();

// Page main processing
$help_manager_edit->Page_Main();
?>
...
// Create page object
var help_manager_edit = new ew_Page("help_manager_edit");

// page properties
help_manager_edit.PageID = "edit"; // page ID
var EW_PAGE_ID = help_manager_edit.PageID; // for backward compatibility

// extend page with ValidateForm function
help_manager_edit.ValidateForm = function(fobj) {
	...
	for (i=0; i<rowcnt; i++) {
		...
		if (elm && !ew_HasValue(elm))
			return ew_OnError(this, elm, "Hãy điền trường bắt buộc - Họ và tên");
		elm = fobj.elements["x" + infix + "_Dien_thoai"];
		if (elm && !ew_HasValue(elm))
			return ew_OnError(this, elm, "Hãy điền trường bắt buộc - Số điện thoại");
		elm = fobj.elements["x" + infix + "_chuc_nang"];
		if (elm && !ew_HasValue(elm))
			return ew_OnError(this, elm, "Hãy điền trường bắt buộc - Chức năng");
		elm = fobj.elements["x" + infix + "_zemail"];
		if (elm && !ew_HasValue(elm))
			return ew_OnError(this, elm, "Hãy điền trường bắt buộc - Email");
		elm = fobj.elements["x" + infix + "_Nick_yahoo"];
		if (elm && !ew_HasValue(elm))
			return ew_OnError(this, elm, "Hãy điền trường bắt buộc - Nick Yahoo");
		elm = fobj.elements["x" + infix + "_Nick_Skype"];
		if (elm && !ew_HasValue(elm))
			return ew_OnError(this, elm, "Hãy điền trường bắt buộc - Nick Skype");
...
help_manager_edit.Form_CustomValidate = 
 function(fobj) { // DO NOT CHANGE THIS LINE!

 	// Your custom validation code here, return false if invalid. 
 	return true;
 }
help_manager_edit.SelectAllKey = function(elem) {
	ew_SelectAll(elem);
}
<?php if (EW_CLIENT_VALIDATE) { ?>
help_manager_edit.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
help_manager_edit.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
<table border="0" width="100%" id="table806" cellspacing="0" cellpadding="0">
							<tr>
								...
								&nbsp;&nbsp;&nbsp;Sửa thông tin người trợ giúp</font></b></td>
...
<?php $help_manager_edit->ShowMessage() ?>
<form name="fhelp_manageredit" id="fhelp_manageredit" action="<?php echo ew_CurrentPage() ?>" method="post" onsubmit="return help_manager_edit.ValidateForm(this);">
<p>
<input type="hidden" name="a_table" id="a_table" value="help_manager">
...
<input type="hidden" name="x_help_id" id="x_help_id" value="<?php echo ew_HtmlEncode($help_manager->help_id->CurrentValue) ?>">
<p>
...
class chelp_manager_edit {
	...
	// Page Object Name
	var $PageObjName = 'help_manager_edit';
	...
	function Page_Main() {
		...
		if (@$_POST["a_edit"] <> "") {
			$help_manager->CurrentAction = $_POST["a_edit"]; // Get action code
			$this->LoadFormValues(); // Get form values

			// Validate Form
			if (!$this->ValidateForm()) {
				$help_manager->CurrentAction = ""; // Form error, reset action
				$this->setMessage($gsFormError);
				$this->RestoreFormValues();
			}
		} else {
			$help_manager->CurrentAction = "I"; // Default action is display
		}

		// Check if valid key
		if ($help_manager->help_id->CurrentValue == "")
			$this->Page_Terminate("help_managerlist.php"); // Invalid key, return to list
		switch ($help_manager->CurrentAction) {
			case "I": // Get a record to display
				...
				break;
			Case "U": // Update
				$help_manager->SendEmail = TRUE; // Send email on update success
		...
		// Render the record
		$help_manager->RowType = EW_ROWTYPE_EDIT; // Render as edit
		$this->RenderRow();
	}
	...
	function RenderRow() {
		...
		if ($help_manager->RowType == EW_ROWTYPE_VIEW) { // View row
		...
		} elseif ($help_manager->RowType == EW_ROWTYPE_EDIT) { // Edit row

			// help_id
			$help_manager->help_id->EditCustomAttributes = "";
			$help_manager->help_id->EditValue = $help_manager->help_id->CurrentValue;
			$help_manager->help_id->CssStyle = "";
			$help_manager->help_id->CssClass = "";
			$help_manager->help_id->ViewCustomAttributes = "";
			...
			$arwrk = array();
			$arwrk[] = array("1", "Quản lý website");
			$arwrk[] = array("2", "Chăm sóc khách hàng");
...
*/
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			$bUpdateRow = $help_manager->Row_Updating($rsold, $rsnew);
			if ($bUpdateRow) {
				$conn->raiseErrorFn = 'ew_ErrorFn';
				$EditRow = $conn->Execute($help_manager->UpdateSQL($rsnew));
				$conn->raiseErrorFn = '';
			} else {
		...
		if ($EditRow)
			$help_manager->Row_Updated($rsold, $rsnew);
		$rs->Close();
		return $EditRow;
	}
