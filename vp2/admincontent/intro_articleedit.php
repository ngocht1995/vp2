...
// Define page object
$intro_article_edit = new cintro_article_edit();
$Page =& $intro_article_edit;

// Page init processing
$intro_article_edit->Page_Init();

// Page main processing
$intro_article_edit->Page_Main();
?>
...
// Create page object
var intro_article_edit = new ew_Page("intro_article_edit");

// page properties
intro_article_edit.PageID = "edit"; // page ID
var EW_PAGE_ID = intro_article_edit.PageID; // for backward compatibility

// extend page with ValidateForm function
intro_article_edit.ValidateForm = function(fobj) {
	if (!this.ValidateRequired)
		return true; // ignore validation
	if (fobj.a_confirm && fobj.a_confirm.value == "F")
		return true;
	var i, elm,elm1, aelm, infix;
...
//		if (elm && !ew_HasValue(elm))
//			return ew_OnError(this, elm, "Chưa nhập - Nội dung");
...
intro_article_edit.Form_CustomValidate = 
 function(fobj) { // DO NOT CHANGE THIS LINE!

 	// Your custom validation code here, return false if invalid. 
 	return true;
 }
intro_article_edit.SelectAllKey = function(elem) {
	ew_SelectAll(elem);
}
<?php if (EW_CLIENT_VALIDATE) { ?>
intro_article_edit.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
intro_article_edit.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
<?php $intro_article_edit->ShowMessage() ?>
<form name="fintro_articleedit" id="fintro_articleedit" action="<?php echo ew_CurrentPage() ?>" method="post"  enctype="multipart/form-data" onsubmit="return intro_article_edit.ValidateForm(this);">
<p>
<input type="hidden" name="a_table" id="a_table" value="intro_article">
...
<?php if ($intro_article->anh_daidien->HrefValue <> "") { ?>
<?php if (!is_null($intro_article->anh_daidien->Upload->DbValue)) { ?>
<a href="<?php echo $intro_article->anh_daidien->HrefValue ?>" target="_blank"><img src="intro_article_anh_daidien_bv.php?baiviet_id=<?php echo $intro_article->baiviet_id->CurrentValue ?>" border=0<?php echo $intro_article->anh_daidien->ViewAttributes() ?>></a>
<?php } elseif (!in_array($intro_article->CurrentAction, array("I", "edit", "gridedit"))) { ?>
&nbsp;
<?php } ?>
<?php } else { ?>
<?php if (!is_null($intro_article->anh_daidien->Upload->DbValue)) { ?>
<img src="intro_article_anh_daidien_bv.php?baiviet_id=<?php echo $intro_article->baiviet_id->CurrentValue ?>" border=0<?php echo $intro_article->anh_daidien->ViewAttributes() ?>>
<?php } elseif (!in_array($intro_article->CurrentAction, array("I", "edit", "gridedit"))) { ?>
&nbsp;
<?php } ?>
<?php } ?>
...
<?php } ?>
<input type="file" name="x_anh_daidien" id="x_anh_daidien" size="30" onchange="if (this.form.a_anh_daidien[2]) this.form.a_anh_daidien[2].checked=true;"<?php echo $intro_article->anh_daidien->EditAttributes() ?>>
</div>
...
<select id="x_thutu_sapxep" name="x_thutu_sapxep"<?php echo ew_TruncateMemo($intro_article->thutu_sapxep->EditAttributes(), 100) ?>>
<?php
...
<input type="hidden" name="x_baiviet_id" id="x_baiviet_id" value="<?php echo ew_HtmlEncode($intro_article->baiviet_id->CurrentValue) ?>">
<p>
<input type="button" name="btnAction" id="btnAction" value="   Sửa   " onclick="ew_SubmitForm(intro_article_edit, this.form);">
</form>
<script type="text/javascript">
<!--
...
class cintro_article_edit {
	...
	// Page Object Name
	var $PageObjName = 'intro_article_edit';
	...
	function Page_Main() {
		...
		if (@$_POST["a_edit"] <> "") {
			$intro_article->CurrentAction = $_POST["a_edit"]; // Get action code
                        $this->GetUploadFiles(); // Get upload files
			$this->LoadFormValues(); // Get form values

			// Validate Form
			if (!$this->ValidateForm()) {
				$intro_article->CurrentAction = ""; // Form error, reset action
				$this->setMessage($gsFormError);
				$this->RestoreFormValues();
			}
		} else {
			$intro_article->CurrentAction = "I"; // Default action is display
		}

		// Check if valid key
		if ($intro_article->baiviet_id->CurrentValue == "")
			$this->Page_Terminate("intro_articlelist.php"); // Invalid key, return to list
		switch ($intro_article->CurrentAction) {
			case "I": // Get a record to display
				...
				break;
			Case "U": // Update
				$intro_article->SendEmail = TRUE; // Send email on update success
		...
		// Render the record
		$intro_article->RowType = EW_ROWTYPE_EDIT; // Render as edit
		$this->RenderRow();
	}
	...
	function RenderRow() {
		...
		if ($intro_article->RowType == EW_ROWTYPE_VIEW) { // View row
			...
			$arwrk = ($rswrk) ? $rswrk->GetRows() : array();
			if ($rswrk) $rswrk->Close();
			array_unshift($arwrk, array(-1, "--Chọn thứ tự sắp xếp--"));
...
//		if ($intro_article->noidung_baiviet->FormValue == "") {
//			$gsFormError .= ($gsFormError <> "") ? "<br>" : "";
//			$gsFormError .= "Chưa nhập - Nội dung";
//		}
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			if ($intro_article->anh_daidien->Upload->Action == "2" || $intro_article->anh_daidien->Upload->Action == "3") { // Update/Remove
