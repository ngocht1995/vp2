...
// PHPMaker 6 configuration for Table user_new
$user_new = NULL; // Initialize table object
...
class cuser_new {
	...
	function cuser_new() {
		$this->TableVar = "user_new";
		$this->TableName = "user_new";
		$this->SelectLimit = TRUE;
		$this->tintuc_id = new cField('user_new', 'x_tintuc_id', 'tintuc_id', "`tintuc_id`", 19, -1, FALSE);
		$this->fields['tintuc_id'] =& $this->tintuc_id;
		$this->nguoidung_id = new cField('user_new', 'x_nguoidung_id', 'nguoidung_id', "`nguoidung_id`", 19, -1, FALSE);
		$this->fields['nguoidung_id'] =& $this->nguoidung_id;
		$this->tieude_tintuc = new cField('user_new', 'x_tieude_tintuc', 'tieude_tintuc', "`tieude_tintuc`", 200, -1, FALSE);
		$this->fields['tieude_tintuc'] =& $this->tieude_tintuc;
		$this->tukhoa_tintuc = new cField('user_new', 'x_tukhoa_tintuc', 'tukhoa_tintuc', "`tukhoa_tintuc`", 200, -1, FALSE);
		$this->fields['tukhoa_tintuc'] =& $this->tukhoa_tintuc;
		$this->tomtat_tintuc = new cField('user_new', 'x_tomtat_tintuc', 'tomtat_tintuc', "`tomtat_tintuc`", 201, -1, FALSE);
		$this->fields['tomtat_tintuc'] =& $this->tomtat_tintuc;
		$this->anh_tintuc = new cField('user_new', 'x_anh_tintuc', 'anh_tintuc', "`anh_tintuc`", 200, -1, TRUE);
		$this->fields['anh_tintuc'] =& $this->anh_tintuc;
		$this->nguon_tintuc = new cField('user_new', 'x_nguon_tintuc', 'nguon_tintuc', "`nguon_tintuc`", 200, -1, FALSE);
		$this->fields['nguon_tintuc'] =& $this->nguon_tintuc;
		$this->noidung_tintuc = new cField('user_new', 'x_noidung_tintuc', 'noidung_tintuc', "`noidung_tintuc`", 201, -1, FALSE);
		$this->fields['noidung_tintuc'] =& $this->noidung_tintuc;
		$this->lienket_tintuc = new cField('user_new', 'x_lienket_tintuc', 'lienket_tintuc', "`lienket_tintuc`", 200, -1, FALSE);
		$this->fields['lienket_tintuc'] =& $this->lienket_tintuc;
		$this->hienthi_tungay = new cField('user_new', 'x_hienthi_tungay', 'hienthi_tungay', "`hienthi_tungay`", 135, 7, FALSE);
		$this->fields['hienthi_tungay'] =& $this->hienthi_tungay;
		$this->hienthi_denngay = new cField('user_new', 'x_hienthi_denngay', 'hienthi_denngay', "`hienthi_denngay`", 135, 7, FALSE);
		$this->fields['hienthi_denngay'] =& $this->hienthi_denngay;
		$this->thoigian_them = new cField('user_new', 'x_thoigian_them', 'thoigian_them', "`thoigian_them`", 135, 7, FALSE);
		$this->fields['thoigian_them'] =& $this->thoigian_them;
		$this->thoigian_sua = new cField('user_new', 'x_thoigian_sua', 'thoigian_sua', "`thoigian_sua`", 135, 7, FALSE);
		$this->fields['thoigian_sua'] =& $this->thoigian_sua;
		$this->soluot_truynhap = new cField('user_new', 'x_soluot_truynhap', 'soluot_truynhap', "`soluot_truynhap`", 19, -1, FALSE);
		$this->fields['soluot_truynhap'] =& $this->soluot_truynhap;
		$this->trang_thai = new cField('user_new', 'x_trang_thai', 'trang_thai', "`trang_thai`", 19, -1, FALSE);
		$this->fields['trang_thai'] =& $this->trang_thai;
		$this->thutu_sapxep = new cField('user_new', 'x_thutu_sapxep', 'thutu_sapxep', "`thutu_sapxep`", 5, -1, FALSE);
		$this->fields['thutu_sapxep'] =& $this->thutu_sapxep;
		$this->xuatban = new cField('user_new', 'x_xuatban', 'xuatban', "`xuatban`", 19, -1, FALSE);
	...
	function InsertSQL(&$rs) {
		...
		return "INSERT INTO `user_new` ($names) VALUES ($values)";
	}

	// UPDATE statement
	function UpdateSQL(&$rs) {
		$SQL = "UPDATE `user_new` SET ";
		foreach ($rs as $name => $value) {
	...
	function DeleteSQL(&$rs) {
		$SQL = "DELETE FROM `user_new` WHERE ";
	...
	// View url
	function ViewUrl() {
		return $this->KeyUrl("user_newview.php", $this->UrlParm());
	}

	// Add url
	function AddUrl() {
		$AddUrl = "user_newadd.php";
	...
	// Edit url
	function EditUrl() {
		return $this->KeyUrl("user_newedit.php", $this->UrlParm());
	}
	...
	// Copy url
	function CopyUrl() {
		return $this->KeyUrl("user_newadd.php", $this->UrlParm());
	}
	...
	// Delete url
	function DeleteUrl() {
		return $this->KeyUrl("user_newdelete.php", $this->UrlParm());
	}
	...
	function UrlParm($parm = "") {
		$UrlParm = ($this->UseTokenInUrl) ? "t=user_new" : "";
		if ($parm <> "") {
			if ($UrlParm <> "")
				$UrlParm .= "&";
			$UrlParm .= $parm;
	...
	function GetUserIDSubquery(&$fld, &$masterfld) {
		global $conn;
		$sWrk = "";
		$sSql = "SELECT " . $masterfld->FldExpression . " FROM `user_new` WHERE " . $this->AddUserIDFilter("");
