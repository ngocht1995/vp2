...
<table border="0" width="100%" id="table806" cellspacing="0" cellpadding="0">
							<tr>
								...
								&nbsp;&nbsp;&nbsp;Thêm thông tin sản phẩm</font></b></td>
...
<input type="file" name="x_anh_to" id="x_anh_to" size="30"<?php echo $products->anh_to->EditAttributes() ?>>
</div>
...
<input type="file" name="x_anh_nho" id="x_anh_nho" size="30"<?php echo $products->anh_nho->EditAttributes() ?>>
</div>
...
<p>
<input type="button" name="btnAction" id="btnAction" value="    Thêm mới    " onclick="ew_SubmitForm(products_add, this.form);">
</form>
...
class cproducts_add {
	...
	//
	function Page_Main() {
		global $objForm, $gsFormError, $products,$conn;
		...
		switch ($products->CurrentAction) {
		  ...
		  case "A": // ' Add new record
		    ...
		    if ($this->AddRow()) { // Add successful
		     // $this->setMessage("Đã thêm sản phẩm"); // Set up success message
                       $this->setMessage("");
                      $str="SELECT sanpham_id FROM products where nguoidung_id=".CurrentUserID()." order by  sanpham_id desc limit 1";
                        $r1 = $conn->Execute($str);
		 // Close Connection
                        $url="pic_productlist.php?showmaster=products&sanpham_id=".$r1->fields['sanpham_id'];



					$sReturnUrl =$url;// $products->getReturnUrl();
	...
	function LoadDefaultValues() {
		global $products;
                $products->xuat_su->CurrentValue = VN;
                $products->loai_tien->CurrentValue = 0;
                $products->thanhtoan_status->CurrentValue=0;
		$products->comment_status->CurrentValue = 0;		
	}
	...
	function LoadFormValues() {
		...
		$products->tg_themsanpham->setFormValue($objForm->GetValue("x_tg_themsanpham"));
		$products->tg_themsanpham->CurrentValue = ew_UnFormatDateTime($products->tg_themsanpham->CurrentValue, 7);
	...
	function RestoreFormValues() {
		...
		$products->tg_themsanpham->CurrentValue = $products->tg_themsanpham->FormValue;
		$products->tg_themsanpham->CurrentValue = ew_UnFormatDateTime($products->tg_themsanpham->CurrentValue, 7);
...
$img = new Securimage();
	...
	function AddRow() {
		...
		// Field tg_themsanpham
		$products->tg_themsanpham->SetDbValueDef(ew_CurrentDateTime(), NULL);
		$rsnew['tg_themsanpham'] =& $products->tg_themsanpham->DbValue;
		...
		if ($AddRow) {
			$products->sanpham_id->setDbValue($conn->Insert_ID());
			$rsnew['sanpham_id'] =& $products->sanpham_id->DbValue;
