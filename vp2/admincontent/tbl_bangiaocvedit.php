...
// Define page object
$tbl_bangiaocv_edit = new ctbl_bangiaocv_edit();
$Page =& $tbl_bangiaocv_edit;

// Page init processing
$tbl_bangiaocv_edit->Page_Init();

// Page main processing
$tbl_bangiaocv_edit->Page_Main();
?>
...
// Create page object
var tbl_bangiaocv_edit = new ew_Page("tbl_bangiaocv_edit");

// page properties
tbl_bangiaocv_edit.PageID = "edit"; // page ID
var EW_PAGE_ID = tbl_bangiaocv_edit.PageID; // for backward compatibility

// extend page with ValidateForm function
tbl_bangiaocv_edit.ValidateForm = function(fobj) {
...
tbl_bangiaocv_edit.Form_CustomValidate = 
 function(fobj) { // DO NOT CHANGE THIS LINE!

 	// Your custom validation code here, return false if invalid. 
 	return true;
 }
tbl_bangiaocv_edit.SelectAllKey = function(elem) {
	ew_SelectAll(elem);
}
<?php if (EW_CLIENT_VALIDATE) { ?>
tbl_bangiaocv_edit.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
tbl_bangiaocv_edit.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
<?php $tbl_bangiaocv_edit->ShowMessage() ?>
<form name="ftbl_bangiaocvedit" id="ftbl_bangiaocvedit" action="<?php echo ew_CurrentPage() ?>" method="post">
<p>
<input type="hidden" name="a_table" id="a_table" value="tbl_bangiaocv">
...
<input type="hidden" name="x_bangiao_id" id="x_bangiao_id" value="<?php echo ew_HtmlEncode($tbl_bangiaocv->bangiao_id->CurrentValue) ?>">
<p>
<input type="button" name="btnAction" id="btnAction" value="   Edit   " onclick="ew_SubmitForm(tbl_bangiaocv_edit, this.form);">
</form>
<script type="text/javascript">
<!--
...
class ctbl_bangiaocv_edit {
	...
	// Page Object Name
	var $PageObjName = 'tbl_bangiaocv_edit';
	...
	function Page_Main() {
		...
		if (@$_POST["a_edit"] <> "") {
			$tbl_bangiaocv->CurrentAction = $_POST["a_edit"]; // Get action code
			$this->LoadFormValues(); // Get form values

			// Validate Form
			if (!$this->ValidateForm()) {
				$tbl_bangiaocv->CurrentAction = ""; // Form error, reset action
				$this->setMessage($gsFormError);
				$this->RestoreFormValues();
			}
		} else {
			$tbl_bangiaocv->CurrentAction = "I"; // Default action is display
		}

		// Check if valid key
		if ($tbl_bangiaocv->bangiao_id->CurrentValue == "")
			$this->Page_Terminate("tbl_bangiaocvlist.php"); // Invalid key, return to list
		switch ($tbl_bangiaocv->CurrentAction) {
			case "I": // Get a record to display
				...
				break;
			Case "U": // Update
				$tbl_bangiaocv->SendEmail = TRUE; // Send email on update success
		...
		// Render the record
		$tbl_bangiaocv->RowType = EW_ROWTYPE_EDIT; // Render as edit
		$this->RenderRow();
	}
	...
	function RenderRow() {
		...
		} elseif ($tbl_bangiaocv->RowType == EW_ROWTYPE_EDIT) { // Edit row
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			$bUpdateRow = $tbl_bangiaocv->Row_Updating($rsold, $rsnew);
			if ($bUpdateRow) {
				$conn->raiseErrorFn = 'ew_ErrorFn';
				$EditRow = $conn->Execute($tbl_bangiaocv->UpdateSQL($rsnew));
				$conn->raiseErrorFn = '';
			} else {
		...
		if ($EditRow)
			$tbl_bangiaocv->Row_Updated($rsold, $rsnew);
		$rs->Close();
		return $EditRow;
	}
