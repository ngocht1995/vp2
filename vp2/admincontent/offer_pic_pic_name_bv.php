...
// Define page object
$offer_pic_pic_name_blobview = new coffer_pic_pic_name_blobview();
$Page =& $offer_pic_pic_name_blobview;

// Page init processing
$offer_pic_pic_name_blobview->Page_Init();

// Page main processing
$offer_pic_pic_name_blobview->Page_Main();
?>
<?php
...
// Page Class
//
class coffer_pic_pic_name_blobview {

	// Page ID
	var $PageID = 'blobview';

	// Page Object Name
	var $PageObjName = 'offer_pic_pic_name_blobview';
	...
	//  - open connection
	//
	function coffer_pic_pic_name_blobview() {
		global $conn;
	...
	function Page_Init() {
		...
		$Security->LoadCurrentUserLevel('offer_pic');
	...
	function Page_Main() {
		...
		$objBinary = new cUpload('offer_pic', 'x_pic_name');
		...
		if ($rs = $conn->Execute($sSql)) {
			if (!$rs->EOF) {
				...
				if (trim(strval($rs->fields('pic_name'))) <> "") {
					header("Content-Disposition: attachment; filename=" . $rs->fields('pic_name'));
				}
				$objBinary->Value = $rs->fields('pic_name');
				if ($bShowThumbnail) {
