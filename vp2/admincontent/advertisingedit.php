...
advertising_edit.ValidateForm = function(fobj) {
	...
	for (i=0; i<rowcnt; i++) {
		...
		if (elm && !ew_HasValue(elm))
			return ew_OnError(this, elm, "Chưa nhập - Luachon Hienthi");
...
<table border="0" width="100%" id="table806" cellspacing="0" cellpadding="0">
							<tr>
								...
								&nbsp;&nbsp;&nbsp;Sửa danh mục quảng cáo</font></b></td>
...
<?php $advertising_edit->ShowMessage() ?>
...
<?php  } ?>
<input type="file" name="x_anh_logo" id="x_anh_logo" size="30" onchange="if (this.form.a_anh_logo[2]) this.form.a_anh_logo[2].checked=true;"<?php echo $advertising->anh_logo->EditAttributes() ?>>
</div>
...
<input type="hidden" name="x_lienket_id" id="x_lienket_id" value="<?php echo ew_HtmlEncode($advertising->lienket_id->CurrentValue) ?>">
<p>
...
class cadvertising_edit {
	...
	function Page_Main() {
		...
		// Check if valid key
		if ($advertising->lienket_id->CurrentValue == "")
	...
	function ValidateForm() {
		...
		if ($advertising->anh_logo->Upload->Action == "3" && is_null($advertising->anh_logo->Upload->Value)) {
			$gsFormError .= ($gsFormError <> "") ? "<br>" : "";
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			if ($advertising->anh_logo->Upload->Action == "2" || $advertising->anh_logo->Upload->Action == "3") { // Update/Remove
			...
			{
				// Field dorong_anh
			$advertising->dorong_anh->SetDbValueDef($advertising->dorong_anh->CurrentValue, 0);
			$rsnew['dorong_anh'] =& $advertising->dorong_anh->DbValue;

			// Field chieucao_anh
			$advertising->chieucao_anh->SetDbValueDef($advertising->chieucao_anh->CurrentValue, 0);
			$rsnew['chieucao_anh'] =& $advertising->chieucao_anh->DbValue;
			}
