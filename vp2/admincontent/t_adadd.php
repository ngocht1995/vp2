...
// Create page object
var t_ad_add = new ew_Page("t_ad_add");

// page properties
t_ad_add.PageID = "add"; // page ID
var EW_PAGE_ID = t_ad_add.PageID; // for backward compatibility

// extend page with ValidateForm function
t_ad_add.ValidateForm = function(fobj) {
...
t_ad_add.Form_CustomValidate = 
 function(fobj) { // DO NOT CHANGE THIS LINE!

 	// Your custom validation code here, return false if invalid. 
 	return true;
 }
t_ad_add.SelectAllKey = function(elem) {
	ew_SelectAll(elem);
}
<?php if (EW_CLIENT_VALIDATE) { ?>
t_ad_add.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
t_ad_add.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
<p><span class="phpmaker"><table border="0" width="100%" id="table806" cellspacing="0" cellpadding="0">
							<tr>
								...
								&nbsp;&nbsp;&nbsp;Thêm tin rao vặt</font></b></td>
								<td height="20" width="54%" background="images/bg-line.gif" align="right" valign="top">
				&nbsp;</td>
							</tr>
							
</table></span></p>
<?php $t_ad_add->ShowMessage() ?>
<form name="ft_adadd" id="ft_adadd" action="<?php echo ew_CurrentPage() ?>" method="post">
<p>
...
<input type="text" name="x_date_c" id="x_date_c" value="<?php echo $t_ad->date_c->EditValue ?>"<?php echo $t_ad->date_c->EditAttributes() ?>>
</span><?php echo $t_ad->date_c->CustomMsg ?>&nbsp;<img src="images/calendar.png" id="cal_x_date_cread_time" name="cal_x_date_cread_time" alt="Pick a date" style="cursor:pointer;cursor:hand;">
<script type="text/javascript">
Calendar.setup({
...
<p>
<input type="button" name="btnAction" id="btnAction" value="    Thêm     " onclick="ew_SubmitForm(t_ad_add, this.form);">
</form>
<script type="text/javascript">
<!--
...
class ct_ad_add {
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
		   $t_ad->CurrentAction = $_POST["a_add"]; // Get form action
		  $this->LoadFormValues(); // Load form values

			// Validate Form
			if (!$this->ValidateForm()) {
				$t_ad->CurrentAction = "I"; // Form error, reset action
				$this->setMessage($gsFormError);
			}
		} else { // Not post back
		  if ($bCopy) {
		    $t_ad->CurrentAction = "C"; // Copy Record
		  } else {
		    $t_ad->CurrentAction = "I"; // Display Blank Record
		...
		switch ($t_ad->CurrentAction) {
		  ...
		  case "A": // ' Add new record
				$t_ad->SendEmail = TRUE; // Send email on add success
		...
		$t_ad->RowType = EW_ROWTYPE_ADD;  // Render add type
	...
	function RenderRow() {
		...
		} elseif ($t_ad->RowType == EW_ROWTYPE_ADD) { // Add row
	...
	function AddRow() {
		...
		$bInsertRow = $t_ad->Row_Inserting($rsnew);
		if ($bInsertRow) {
			$conn->raiseErrorFn = 'ew_ErrorFn';
			$AddRow = $conn->Execute($t_ad->InsertSQL($rsnew));
			$conn->raiseErrorFn = '';
		} else {
		...
		if ($AddRow) {
			$t_ad->ad_ID->setDbValue($conn->Insert_ID());
			$rsnew['ad_ID'] =& $t_ad->ad_ID->DbValue;

			// Call Row Inserted event
			$t_ad->Row_Inserted($rsnew);
		}
		return $AddRow;
	}
...
if( $_SESSION['htsv_status_UserID']!="")
          if (!is_dir("../raovat/upload/users/".CurrentUserID()."")){ mkdir("../raovat/upload/users/".CurrentUserID());}
