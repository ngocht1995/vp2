...
// Define page object
$tbl_doncaithiendiem_add = new ctbl_doncaithiendiem_add();
$Page =& $tbl_doncaithiendiem_add;

// Page init processing
$tbl_doncaithiendiem_add->Page_Init();

// Page main processing
$tbl_doncaithiendiem_add->Page_Main();
?>
...
// Create page object
var tbl_doncaithiendiem_add = new ew_Page("tbl_doncaithiendiem_add");

// page properties
tbl_doncaithiendiem_add.PageID = "add"; // page ID
var EW_PAGE_ID = tbl_doncaithiendiem_add.PageID; // for backward compatibility

// extend page with ValidateForm function
tbl_doncaithiendiem_add.ValidateForm = function(fobj) {
...
tbl_doncaithiendiem_add.Form_CustomValidate = 
 function(fobj) { // DO NOT CHANGE THIS LINE!

 	// Your custom validation code here, return false if invalid. 
 	return true;
 }
tbl_doncaithiendiem_add.SelectAllKey = function(elem) {
	ew_SelectAll(elem);
}
<?php if (EW_CLIENT_VALIDATE) { ?>
tbl_doncaithiendiem_add.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
tbl_doncaithiendiem_add.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
<?php $tbl_doncaithiendiem_add->ShowMessage() ?>
<form name="ftbl_doncaithiendiemadd" id="ftbl_doncaithiendiemadd" action="<?php echo ew_CurrentPage() ?>" method="post" onsubmit="return tbl_doncaithiendiem_add.ValidateForm(this);">
<p>
...
<p>
<input type="submit" name="btnAction" id="btnAction" value="    Xác nhận tạo đơn cải thiện điểm    ">
</form>
...
// Page Class
//
class ctbl_doncaithiendiem_add {
	...
	// Page Object Name
	var $PageObjName = 'tbl_doncaithiendiem_add';
	...
	//  - open connection
	//
	function ctbl_doncaithiendiem_add() {
		global $conn;
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
		   $tbl_doncaithiendiem->CurrentAction = $_POST["a_add"]; // Get form action
		  ...
		  if ($bCopy) {
		    $tbl_doncaithiendiem->CurrentAction = "C"; // Copy Record
		  } else {
		    $tbl_doncaithiendiem->CurrentAction = "I"; // Display Blank Record
		...
		switch ($tbl_doncaithiendiem->CurrentAction) {
		  ...
		  case "A": // ' Add new record
				$tbl_doncaithiendiem->SendEmail = TRUE; // Send email on add success
		...
		$tbl_doncaithiendiem->RowType = EW_ROWTYPE_ADD;  // Render add type
	...
	function LoadFormValues() {
		...
		$tbl_doncaithiendiem->date_time_add->setFormValue($objForm->GetValue("x_date_time_add"));
		$tbl_doncaithiendiem->date_time_add->CurrentValue = ew_UnFormatDateTime($tbl_doncaithiendiem->date_time_add->CurrentValue, 7);
	...
	function RestoreFormValues() {
		...
		$tbl_doncaithiendiem->date_time_add->CurrentValue = $tbl_doncaithiendiem->date_time_add->FormValue;
		$tbl_doncaithiendiem->date_time_add->CurrentValue = ew_UnFormatDateTime($tbl_doncaithiendiem->date_time_add->CurrentValue, 7);
	...
	function RenderRow() {
		...
		} elseif ($tbl_doncaithiendiem->RowType == EW_ROWTYPE_ADD) { // Add row
	...
	function AddRow() {
		...
		$sFilter = $tbl_doncaithiendiem->KeyFilter();
		if ($bCheckKey) {
			$rsChk = $tbl_doncaithiendiem->LoadRs($sFilter);
			if ($rsChk && !$rsChk->EOF) {
		...
		// Field ngay_tao_don
		$tbl_doncaithiendiem->ngay_tao_don->SetDbValueDef(ew_CurrentDateTime(), NULL);
		...
		// Field status
		$tbl_doncaithiendiem->status->SetDbValueDef(1, NULL);
		...
		$bInsertRow = $tbl_doncaithiendiem->Row_Inserting($rsnew);
		if ($bInsertRow) {
			$conn->raiseErrorFn = 'ew_ErrorFn';
			$AddRow = $conn->Execute($tbl_doncaithiendiem->InsertSQL($rsnew));
			$conn->raiseErrorFn = '';
		} else {
		...
		if ($AddRow) {
			$tbl_doncaithiendiem->phieucaithiendiem_id->setDbValue($conn->Insert_ID());
			$rsnew['phieucaithiendiem_id'] =& $tbl_doncaithiendiem->phieucaithiendiem_id->DbValue;

			// Call Row Inserted event
			$tbl_doncaithiendiem->Row_Inserted($rsnew);
