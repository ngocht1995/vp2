...
// Create page object
var rss_edit = new ew_Page("rss_edit");

// page properties
rss_edit.PageID = "edit"; // page ID
var EW_PAGE_ID = rss_edit.PageID; // for backward compatibility

// extend page with ValidateForm function
rss_edit.ValidateForm = function(fobj) {
...
rss_edit.Form_CustomValidate = 
 function(fobj) { // DO NOT CHANGE THIS LINE!

 	// Your custom validation code here, return false if invalid. 
 	return true;
 }
rss_edit.SelectAllKey = function(elem) {
	ew_SelectAll(elem);
}
<?php if (EW_CLIENT_VALIDATE) { ?>
rss_edit.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
rss_edit.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
<?php $rss_edit->ShowMessage() ?>
<form name="frssedit" id="frssedit" action="<?php echo ew_CurrentPage() ?>" method="post" onsubmit="return rss_edit.ValidateForm(this);">
<p>
<input type="hidden" name="a_table" id="a_table" value="rss">
...
<?php if ($rss->rss_order->Visible) { // rss_order ?>
	<tr<?php echo $rss->rss_order->RowAttributes ?>>
		<td class="ewTableHeader">Thứ tự RSS<span class="ewRequired">&nbsp;*</span></td>
...
<input type="hidden" name="x_rss_id" id="x_rss_id" value="<?php echo ew_HtmlEncode($rss->rss_id->CurrentValue) ?>">
<p>
...
class crss_edit {
	...
	function Page_Main() {
		...
		if (@$_POST["a_edit"] <> "") {
			$rss->CurrentAction = $_POST["a_edit"]; // Get action code
			$this->LoadFormValues(); // Get form values

			// Validate Form
			if (!$this->ValidateForm()) {
				$rss->CurrentAction = ""; // Form error, reset action
				$this->setMessage($gsFormError);
				$this->RestoreFormValues();
			}
		} else {
			$rss->CurrentAction = "I"; // Default action is display
		}

		// Check if valid key
		if ($rss->rss_id->CurrentValue == "")
			$this->Page_Terminate("rsslist.php"); // Invalid key, return to list
		switch ($rss->CurrentAction) {
			case "I": // Get a record to display
				...
				break;
			Case "U": // Update
				$rss->SendEmail = TRUE; // Send email on update success
		...
		// Render the record
		$rss->RowType = EW_ROWTYPE_EDIT; // Render as edit
		$this->RenderRow();
	}
	...
	function RenderRow() {
		...
		} elseif ($rss->RowType == EW_ROWTYPE_EDIT) { // Edit row
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			$bUpdateRow = $rss->Row_Updating($rsold, $rsnew);
			if ($bUpdateRow) {
				$conn->raiseErrorFn = 'ew_ErrorFn';
				$EditRow = $conn->Execute($rss->UpdateSQL($rsnew));
				$conn->raiseErrorFn = '';
			} else {
