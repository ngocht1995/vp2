...
// Define page object
$t_manager_services_edit = new ct_manager_services_edit();
$Page =& $t_manager_services_edit;

// Page init processing
$t_manager_services_edit->Page_Init();

// Page main processing
$t_manager_services_edit->Page_Main();
?>
...
// Create page object
var t_manager_services_edit = new ew_Page("t_manager_services_edit");

// page properties
t_manager_services_edit.PageID = "edit"; // page ID
var EW_PAGE_ID = t_manager_services_edit.PageID; // for backward compatibility

// extend page with ValidateForm function
t_manager_services_edit.ValidateForm = function(fobj) {
...
t_manager_services_edit.Form_CustomValidate = 
 function(fobj) { // DO NOT CHANGE THIS LINE!

 	// Your custom validation code here, return false if invalid. 
 	return true;
 }
t_manager_services_edit.SelectAllKey = function(elem) {
	ew_SelectAll(elem);
}
<?php if (EW_CLIENT_VALIDATE) { ?>
t_manager_services_edit.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
t_manager_services_edit.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
<?php $t_manager_services_edit->ShowMessage() ?>
<form name="ft_manager_servicesedit" id="ft_manager_servicesedit" action="<?php echo ew_CurrentPage() ?>" method="post">
<p>
<input type="hidden" name="a_table" id="a_table" value="t_manager_services">
...
<?php if ($t_manager_services->active_ser->Visible) { // active_ser ?>
	<tr<?php echo $t_manager_services->active_ser->RowAttributes ?>>
		<td class="ewTableHeader">Active Ser<span class="ewRequired">&nbsp;*</span></td>
...
<?php if ($t_manager_services->oder->Visible) { // oder ?>
	<tr<?php echo $t_manager_services->oder->RowAttributes ?>>
		<td class="ewTableHeader">Oder<span class="ewRequired">&nbsp;*</span></td>
...
<input type="hidden" name="x_services_id" id="x_services_id" value="<?php echo ew_HtmlEncode($t_manager_services->services_id->CurrentValue) ?>">
<p>
<input type="button" name="btnAction" id="btnAction" value="   Edit   " onclick="ew_SubmitForm(t_manager_services_edit, this.form);">
</form>
<script type="text/javascript">
<!--
...
// Page Class
//
class ct_manager_services_edit {
	...
	// Page Object Name
	var $PageObjName = 't_manager_services_edit';
	...
	//  - open connection
	//
	function ct_manager_services_edit() {
		global $conn;
	...
	function Page_Main() {
		...
		if (@$_POST["a_edit"] <> "") {
			$t_manager_services->CurrentAction = $_POST["a_edit"]; // Get action code
			$this->LoadFormValues(); // Get form values

			// Validate Form
			if (!$this->ValidateForm()) {
				$t_manager_services->CurrentAction = ""; // Form error, reset action
				$this->setMessage($gsFormError);
				$this->RestoreFormValues();
			}
		} else {
			$t_manager_services->CurrentAction = "I"; // Default action is display
		}

		// Check if valid key
		if ($t_manager_services->services_id->CurrentValue == "")
			$this->Page_Terminate("t_manager_serviceslist.php"); // Invalid key, return to list
		switch ($t_manager_services->CurrentAction) {
			case "I": // Get a record to display
				...
				break;
			Case "U": // Update
				$t_manager_services->SendEmail = TRUE; // Send email on update success
		...
		// Render the record
		$t_manager_services->RowType = EW_ROWTYPE_EDIT; // Render as edit
		$this->RenderRow();
	}
	...
	function LoadFormValues() {
		...
		$t_manager_services->user_edit->setFormValue($objForm->GetValue("x_user_edit"));
		$t_manager_services->datime_edit->setFormValue($objForm->GetValue("x_datime_edit"));
		$t_manager_services->datime_edit->CurrentValue = ew_UnFormatDateTime($t_manager_services->datime_edit->CurrentValue, 7);
	...
	function RestoreFormValues() {
		...
		$t_manager_services->user_edit->CurrentValue = $t_manager_services->user_edit->FormValue;
		$t_manager_services->datime_edit->CurrentValue = $t_manager_services->datime_edit->FormValue;
		$t_manager_services->datime_edit->CurrentValue = ew_UnFormatDateTime($t_manager_services->datime_edit->CurrentValue, 7);
	...
	function RenderRow() {
		...
		// user_edit
		$t_manager_services->user_edit->CellCssStyle = "";
		$t_manager_services->user_edit->CellCssClass = "";

		// datime_edit
		$t_manager_services->datime_edit->CellCssStyle = "";
		$t_manager_services->datime_edit->CellCssClass = "";
		...
		if ($t_manager_services->RowType == EW_ROWTYPE_VIEW) { // View row
			...
			// user_edit
			$t_manager_services->user_edit->HrefValue = "";

			// datime_edit
			$t_manager_services->datime_edit->HrefValue = "";

			// oder
			$t_manager_services->oder->HrefValue = "";
		} elseif ($t_manager_services->RowType == EW_ROWTYPE_EDIT) { // Edit row
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// Field user_edit
			$t_manager_services->user_edit->SetDbValueDef(CurrentUserID(), NULL);
			$rsnew['user_edit'] =& $t_manager_services->user_edit->DbValue;

			// Field datime_edit
			$t_manager_services->datime_edit->SetDbValueDef(ew_CurrentDateTime(), NULL);
			$rsnew['datime_edit'] =& $t_manager_services->datime_edit->DbValue;
			...
			$bUpdateRow = $t_manager_services->Row_Updating($rsold, $rsnew);
			if ($bUpdateRow) {
				$conn->raiseErrorFn = 'ew_ErrorFn';
				$EditRow = $conn->Execute($t_manager_services->UpdateSQL($rsnew));
				$conn->raiseErrorFn = '';
			} else {
		...
		if ($EditRow)
			$t_manager_services->Row_Updated($rsold, $rsnew);
		$rs->Close();
		return $EditRow;
	}
