 ...
 if(KillChars(htmlspecialchars($_GET['flag'],ENT_QUOTES)) <> '1')  
   {
      ...
      if (isset($result['TKBResult']['diffgram']['DocumentElement']['TKB']))
            ...
            mysql_query("INSERT INTO tbl_tkb (TenGiaoVien, Magiaovien, MaSinhVien, MaLop, MaMonHoc, TenMonHoc, sotc, MaPhongHoc, NamHoc, HocKy, SoTuanHoc, TuanHocBatDau, TuNgay, NgayKetThuc, SoTiet, Thu2, Thu3, Thu4, Thu5, Thu6, Thu7, CN)
                      ...
                      $sSqlWrk = "SELECT max(NgayKetThuc) as root_end FROM tbl_tkb";
                        if ($sWhereWrk <> "") $sSqlWrk .= " WHERE $sWhereWrk";
                        $rswrk = $conn->Execute($sSqlWrk);
                        $arwrk = ($rswrk) ? $rswrk->GetRows() : array();
                        $rowswrk = count($arwrk);
                        $root_end = date ( 'Y-m-j' ,strtotime ($arwrk[0]['root_end']));;
                      //end
                     $sSqlWrk ="";
                    for($i=0;$i<count ($result);$i++)
                    {
...
if ((count($arwrk)>0) && ($no_tkb == 0))
{
    ...
    $array = array();
    $batdau= array();
    $sSqlWrk = "SELECT TuanHocBatDau,SoTuanHoc,(TuanHocBatDau + SoTuanHoc -1) AS gd1 FROM tbl_tkb ORDER BY gd1";
    $rswrk = $conn->Execute($sSqlWrk);
    $arwrk = ($rswrk) ? $rswrk->GetRows() : array();
   // echo $sSqlWrk;
    $rowswrk = count($arwrk);
    ...
    for($i=0;$i<count($arwrk);$i++)
    {
        if(in_array($arwrk[$i]['gd1'],$array)== FALSE)
        {
            array_unshift($array,$arwrk[$i]['gd1']);

        }  
        if(in_array($arwrk[$i]['TuanHocBatDau'],$batdau)== FALSE)
        {
            array_unshift($batdau,$arwrk[$i]['TuanHocBatDau']);
...
//    print_r($array);
//    echo "<br/>";
    $giaidoan = array_merge($array,$batdau);
//    print_r($giaidoan);
    usort($giaidoan, 'cmp');
    $giaidoan_end = $giaidoan;
    for($j=0;$j<count($giaidoan);$j++)
    {
    if((in_array($giaidoan[$j],$batdau)== true) && (in_array($giaidoan[$j+1],$batdau)== true))
        {
            $them= $giaidoan[$j+1]-1;
            array_push($giaidoan_end,$them);
        }  

    }

    usort($giaidoan_end, 'cmp');
 // print_r($giaidoan_end);
    //echo "<br/>";
    $max =count($giaidoan_end);
    if(($max%2 <> 0)) 
    {
        array_push($giaidoan_end,$giaidoan_end[$max-1]); 
        $max = $max+1;
        $faq = 1;
    }
    $tong = 0;
    for($j=0;$j<$max;$j++)
    {
        ...
        {   
        $tong++;
    $sotuan=$giaidoan_end[$j]-$giaidoan_end[$j-1]+1;
    if ($j==1)
    {  
    $array_time_stage =  tkb_begin_end_week($root_begin,$giaidoan_end[$j-1],$giaidoan_end[$j]);
    $tungay =$array_time_stage[0];
    $denngay= $array_time_stage[1];
    $tungay_giadoan = $array_time_stage[2];
 
    }
    else
    {
    $array_time_stage =  tkb_begin_end_week($tungay_giadoan,$giaidoan_end[$j-1],$giaidoan_end[$j]);
    $tungay =$array_time_stage[0];
    $denngay= $array_time_stage[1];
    $tungay_giadoan = $array_time_stage[2];
    }     
    $khoanggd_1 = date ( 'Y-m-j' ,strtotime ($tungay));
    if (($faq==1) && ($j+1==$max))
    { $khoanggd_2 = $root_end;} 
    else 
    { $khoanggd_2 = date ( 'Y-m-j' ,strtotime ($denngay));} 
            if ($khoanggd_1 < $khoanggd_2)
            {
   
    $title= "Từ ".date ( 'j-m-Y' ,strtotime ($khoanggd_1))." đến ".date ( 'j-m-Y' ,strtotime ($khoanggd_2))."";
    $sSql = "SELECT * FROM tbl_tkb WHERE (tbl_tkb.TuNgay >= '".$khoanggd_1."' or  tbl_tkb.NgayKetThuc >= '".$khoanggd_2."') 
        OR (tbl_tkb.NgayKetThuc >= '".$khoanggd_1."' AND  tbl_tkb.NgayKetThuc <= '".$khoanggd_2."')
        OR (tbl_tkb.NgayKetThuc <= '".$khoanggd_1."' AND  tbl_tkb.TuNgay >= '".$khoanggd_2."')";    
    ...
    <td align="left" bgcolor="#66FFFF" width="60">
        <table width="100%" style="border-collapse: collapse;border-spacing:0;height: 165px;">  
    <?php for($i=0;$i<6;$i++)
        {    
            switch ($i) 
            {
        ...
        ?>
            <tr><td  border='1'  align="left" bgcolor="#66FFFF" style="border-bottom:1px #000 solid" width="120"> <a style="cursor: pointer" title="<?php echo $title  ?>">&nbsp;&nbsp;&nbsp;Tiết <?php echo $i+1 ?></a></td></tr>
    <?php } ?>
        </table>
    ...
    <td width='120' bgcolor='#66FFFF'>
        ...
        <td  style="background: <?php echo $a1 ?>;height:82px;"><span style="font-size: 13px;">
            Mã môn:<a href="#" title="<?php echo $arw[$tam]['TenMonHoc'] ?>"><?php echo $arw[$tam]['MaMonHoc'] ?></a>
            ...
            <br/><a class="tkba" target="_blank" href="http://hpu.edu.vn/thoikhoabieu/Tkb_GiaoVien/Tkb_GiaoVien_<?php echo $arw[$tam]['Magiaovien'] ?>.html"><?php echo $arw[$tam]['TenGiaoVien']; ?></a>
            </span>
        </td> 
                <?php } else{   
                    ?>
